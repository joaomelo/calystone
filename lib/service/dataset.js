...
export class Dataset {
  items = reactive(new Map()); // the map data structure will be useful when later the data querying logic needs to deal with events at the document level
  isLoaded = ref(false);
  ...
  constructor({ db, auth, name, where }) {
    ...
    watch(this._auth.user, (user) => this.load(user));
  }
  ...
  del(itemOrItems) {
    const items = asArray(itemOrItems);

    const promises = items.map((itemOrItemId) => {
